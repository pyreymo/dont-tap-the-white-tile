...
public class GameView4 extends View implements View.OnClickListener {
    ...
    public GameView4(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        activity = (Activity) context;
    }
...
//        canvas.drawText("双击", ScrWidth / 2 - 50, 600, RedPaint);
    }

    public void OpenEndDialog() {
        //TODO 做一个更精致的、自定义view的返回界面
    ...
    public void DrawRect(Canvas canvas) {
        ...
        for (int i = 0; i < RectList.size(); i++) {
            ...
            canvas.drawLine(0, rectPosition.getTopPos(), ScrWidth, rectPosition.getTopPos() + 1, GrayPaint);
            canvas.drawRect(rectPosition.getLeftPos(), rectPosition.getTopPos(),
                    rectPosition.getRightPos(), rectPosition.getTopPos() + ScrHeight / (level), BlackPaint);
        }
        //绘制灰块
        for (int i = 0; i < RectList1.size(); i++) {
            rectPosition1 = new RectPosition();
            rectPosition1 = RectList1.get(i);
            canvas.drawLine(0, rectPosition1.getTopPos(), ScrWidth, rectPosition1.getTopPos() + 1, GrayPaint);
            canvas.drawRect(rectPosition1.getLeftPos(), rectPosition1.getTopPos(),
                    rectPosition1.getRightPos(), rectPosition1.getTopPos() + ScrHeight / (level), LTGrayPaint);
        }
    }
    ...
    public void MoveRect() {
        ...
        if (RectList.get(0).getTopPos() >= -speed) {
            // ">=speed" 是为了修正误差，尽管我并不知道为什么这样就没有误差了……
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        for (int i = 0; i < RectList.size(); i++) {
            if (RectList.get(i).getLeftPos() <= X
                    ...
                    && RectList.get(i).getBottomPos() >= Y - ScrHeight / 16
                    && RectList.size() != 1) {
                RectList1.add(RectList.get(i));
                RectList.remove(i);

                score++;
                speed = (int) (15 + score * 0.15);
                //TODO 完善速度模式
            }
        }
